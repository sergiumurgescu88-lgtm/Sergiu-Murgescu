...
import { 
  ...
  CreditCard, ChevronRight, Sparkles, Zap, Bell, AlertCircle, Calendar, Hash, Tag
} from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[1000] flex justify-end bg-black/60 backdrop-blur-sm animate-in fade-in duration-300" onClick={onClose}>
      ...
      >
        <div className="p-8 border-b border-zinc-900 bg-zinc-950/50 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-zinc-500 font-black uppercase tracking-[0.2em] mt-1">Unified Infrastructure Access</p>
            </div>
          </div>
        ...
        <div className="flex px-8 py-4 gap-2 bg-zinc-950 border-b border-zinc-900">
          {[{ id: 'overview', icon: Zap, label: 'Account' }, { id: 'history', icon: Clock, label: 'Credit History' }, { id: 'security', icon: Shield, label: 'Security' }].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveView(tab.id as any)}
              className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeView === tab.id ? 'bg-zinc-900 text-orange-500 border border-orange-500/20 shadow-lg' : 'text-zinc-600 hover:text-zinc-400'}`}
            >
            ...
            <div className="space-y-10 animate-in fade-in">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-zinc-900/40 border border-zinc-800/80 p-6 rounded-[2rem] shadow-xl group hover:border-orange-500/30 transition-all">
                  <div className="flex items-center justify-between mb-3"><Coins className="text-orange-500" size={18} /><ShoppingCart size={12} className="text-zinc-700" /></div>
                  <div className="text-3xl font-serif font-bold text-white">{user.credits}</div>
                  <div className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mt-1">Available Credits</div>
                </div>
                <div className="bg-zinc-900/40 border border-zinc-800/80 p-6 rounded-[2rem] shadow-xl">
                  ...
                  <div className="text-[10px] text-zinc-500 font-black uppercase tracking-widest mt-1">Units Produced</div>
                </div>
              </div>
